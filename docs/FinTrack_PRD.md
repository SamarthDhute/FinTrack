...
- Predefined Payment Method module (required field on every expense, with dashboard breakdown)
...
- **Payment Method** (required) — chosen from a predefined list (see Section 7.4-A)
...
| Payment Method is required (must select from predefined list) | Ensures every expense can be broken down by payment method — no gaps in reporting |
...
Payment Method is a **predefined, fixed list** (not user-editable in V1) and is **required** on every expense — every expense must be assigned exactly one payment method.

**Predefined list (V1):** Cash, Card, UPI, Net Banking, Wallet

| ID | Action | Description | Priority | User Story |
|----|--------|--------------|----------|------------|
| FR-31 | Predefined List | Ship with a fixed list of payment methods (Cash, Card, UPI, Net Banking, Wallet) | P0 | As a user, I want ready-made payment method options so I don't have to set anything up before logging my first expense. |
| FR-32 | Required Assignment | Every expense must have a payment method selected before it can be saved — no expense can exist without one | P0 | As a user, I want payment method to always be captured so my spending breakdown by method is always complete and accurate. |
| FR-33 | View | See the list of payment methods along with how many expenses use each one | P1 | As a user, I want to see how many expenses fall under each payment method so I understand my usage. |
| FR-34 | Dashboard Breakdown | Dashboard shows a chart (pie/donut or bar) of total spend by payment method | P0 | As a user, I want to see how much I spend via Cash vs Card vs UPI so I know which method I rely on most. |

*Note: Custom/user-added payment methods and editing the predefined list are out of scope for V1 — see Section 13, Phase 2.*
...
| FR-15 | Filter — Payment Method | Filter by one or more predefined payment methods (Cash, Card, UPI, Net Banking, Wallet) | P1 | As a user, I want to filter by payment method so I can see how I paid for things. |
...
| FR-35 | Payment method breakdown chart (see FR-34, Section 7.4-A) | Shows which payment method the user relies on most | P0 | As a user, I want to see my spending split by payment method so I know how I'm actually paying for things. |
...
- Every expense has a required Payment Method (from the predefined list), and the Dashboard shows a spend-by-payment-method chart
...
- Payment Method list is fixed/predefined in V1 (Cash, Card, UPI, Net Banking, Wallet) — no custom payment methods until Phase 2
...
| **Phase 2** | Login, Sync & Convenience | Login & multi-device sync, income tracking, recurring expenses (rent, subscriptions, EMI), receipt photo upload, custom/editable payment methods, multiple wallets/accounts (cash, bank, card), report export (PDF/Excel/CSV), dark mode | Each feature tested against real stored data, deployed as an update to Phase 1 app |
