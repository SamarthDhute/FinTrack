...
class CategoryService:
    ...
    def get_all_categories(db: Session) -> List[CategoryResponse]:
        results = CategoryRepository.get_all(db)
        return [
            CategoryResponse(
                id=cat.id,
                name=cat.name,
    ...
    def get_category_by_id(db: Session, category_id: int) -> CategoryResponse:
        ...
        count = CategoryRepository.count_expenses(db, category_id)
    ...
    def create_category(db: Session, data: CategoryCreate) -> CategoryResponse:
        existing = CategoryRepository.get_by_name(db, data.name)
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Category '{data.name}' already exists"
            )
        category = CategoryRepository.create(db, data.name)
        return CategoryResponse(
            id=category.id,
            name=category.name,
            created_at=category.created_at,
